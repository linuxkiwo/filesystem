		...
		*/
		...
		for (let o of obj) {
			...
			if (o["difference_between_front_and_back"]){
				let name_fr = (o["difference_between_front_and_back"] === true) ? "{Render}" : "{ " + o["difference_between_front_and_back"][1] + "}",
					name_bk = (o["difference_between_front_and_back"] === true) ? "Main" : o["difference_between_front_and_back"][0];
				this[where] += `const ${name_fr} = require('${module[0].toLowerCase()}')${module[1]};`;				
				try{
					this.bcknd[`${o["name"]}_${name_bk}`] = require(module[0])[module[1]][name_bk];
				}
				catch(e){console.log(`el módulo o libreria ${module[0]} no se ha encontrado. Por favor, contacte con el adminsitrador del sistema`)}
			}
			else {
